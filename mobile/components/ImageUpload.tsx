...
export function ImageUpload({ value, onChange, aspectRatio = "video", className = "", label, size = "medium" }: ImageUploadProps) {
    ...
    return (
        <View className={`w-full ${className}`}>
            {label && <Text className="text-warm-gray-500 font-semibold mb-2 uppercase text-xs tracking-wider">{label}</Text>}
            <View className={`relative border-2 border-dashed border-warm-gray-200 rounded-xl overflow-hidden bg-warm-gray-50 ${aspectClass} ${size === 'small' && !value ? 'h-24' : ''}`}>
                {value ? (
                    <>
                    ...
                    >
                        {isPicking ? (
                            ...
                            <>
                                ...
                                <Text className="text-warm-gray-500 text-xs font-medium">Upload photo</Text>
                            </>
                        )}
                    </TouchableOpacity>
                )}
