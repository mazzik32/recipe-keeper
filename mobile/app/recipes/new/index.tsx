    ...
    return (
        <SafeAreaView className="flex-1 bg-cream dark:bg-dark-bg dark:bg-dark-bg">
            ...
            <View className="flex-row items-center justify-between px-4 py-3 bg-white dark:bg-dark-card dark:bg-dark-card border-b border-warm-gray-100 dark:border-dark-border dark:border-dark-border">
                ...
                <Text className="font-playfair text-xl text-warm-gray-700 dark:text-dark-text dark:text-dark-text">{t.add.reviewSave}</Text>
            ...
            <ScrollView className="flex-1 px-4 py-6" keyboardDismissMode="on-drag">
                ...
                <View className="mb-8">
                    <View className="flex-row items-center justify-between mb-3">
                        <Text className="font-playfair text-2xl text-warm-gray-700 dark:text-dark-text dark:text-dark-text">Ingredients</Text>
                        <TouchableOpacity className="p-2 bg-peach-100 dark:bg-dark-peach-subtle dark:bg-dark-peach-subtle rounded-full">
                    ...
                    <View className="bg-white dark:bg-dark-card dark:bg-dark-card rounded-2xl p-4 shadow-sm border border-warm-gray-100 dark:border-dark-border dark:border-dark-border">
                        {ingredients.map((ing, idx) => (
                            <View key={idx} className="flex-row items-center py-2 border-b border-warm-gray-50 dark:border-dark-border dark:border-dark-border last:border-0">
                                <TextInput
                                    value={`${ing.quantity || ''} ${ing.unit || ''}`.trim()}
                                    className="w-24 font-semibold text-peach-600 border-r border-warm-gray-100 dark:border-dark-border dark:border-dark-border mr-3"
                                />
                                <TextInput
                                    value={ing.name}
                ...
                <View className="mb-12">
                    <View className="flex-row items-center justify-between mb-3">
                        <Text className="font-playfair text-2xl text-warm-gray-700 dark:text-dark-text dark:text-dark-text">Instructions</Text>
                        <TouchableOpacity className="p-2 bg-peach-100 dark:bg-dark-peach-subtle dark:bg-dark-peach-subtle rounded-full">
                            <Plus color="#eb6e3e" size={20} />
                        </TouchableOpacity>
                    </View>
                        ...
                        {instructions.map((step, idx) => (
                            <View key={idx} className="bg-white dark:bg-dark-card dark:bg-dark-card rounded-2xl p-4 shadow-sm border border-warm-gray-100 dark:border-dark-border dark:border-dark-border flex-row">
                                ...
                                <TextInput
                                    ...
                                    multiline
                                    className="flex-1 text-warm-gray-600 dark:text-dark-text dark:text-dark-text text-base leading-relaxed min-h-[60px]"
                                />
                            </View>
                        ))}
                    </View>
