  ...
  try {
    ...
    if (authError || !user) {
      return new Response(
        JSON.stringify({ 
          success: false,
          error: { message: "Invalid or expired token", details: authError?.message } 
        }),
        { status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    }
...
- For volume: prefer "fl oz", "cup", "pint" or "ml", "L"`;
    ...
    const systemPrompt = `You are a recipe extraction and translation assistant. Your task is to extract recipe information from the provided unstructured text into a structured JSON format.
